...
### 功能介绍
本游戏支持生成微信公众号分享小卡片ticket，让用户可以将游戏成绩以卡片形式分享。

### 使用方法
1. 在游戏结束界面点击"获取分享卡片"按钮
2. 系统会自动调用微信API创建分享卡片ticket
3. 显示ticket和二维码URL信息
4. 可以复制ticket到剪贴板

### 配置要求
在 `config/wechatConfig.js` 中配置您的微信公众号信息：
```javascript
const wechatConfig = {
...
### 相关文件
- `utils/wechatShare.js` - 分享卡片工具类
- `config/wechatConfig.js` - 微信配置
- `docs/wechat-share-card-guide.md` - 详细使用指南
- `examples/wechat-share-example.js` - 使用示例
